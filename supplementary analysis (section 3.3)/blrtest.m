...
%temp2=[];                                     %changed: commented out seven lines
%for y=1:numel(h2)
...
temp2p1=[];                    %changed: added temp2p1 calculation
for y=1:numel(h2)
    temp2p1 = [temp2p1;repmat(p1(y),h2(y),1)];
end

temp2p2=[];                    %changed: added temp2p2 calculation
for y=1:numel(h2)
    temp2p2 = [temp2p2;repmat(p2(y),h2(y),1)];
end

% Calculate log-likelihood at each data point.
%l1 = log(p1(whichbin));  %changed: commented out two lines
%l2 = log(p2(whichbin));
l1 = log(temp2p1);        %changed: added two lines
l2 = log(temp2p2);
