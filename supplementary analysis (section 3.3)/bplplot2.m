...
% Figure parameters
fntsize = 22;             % Font size   %changed: 14
mkrwid = 2;              % Marker width   %changed: remove marker
mkrsize = 8;            % Marker size    %changed: remove marker

temp = get(gca,'ColorOrder');
c1 = temp(1,:);

clrdat = [0 0 0]; % Color for data (black)
clrfit = c1;      % Color for fit  (blue)
lndat = 2;                 % Line width for data
lnfit = 3;                 % Line width for fit

% Draw figure
figure;
set(gca, 'Linestyleorder', '-','fontsize', fntsize, 'yscale', 'log', 'xscale', 'log');
xlabel('Size cutoff', 'fontsize', fntsize);
%ylabel('Pr(X \geq x)', 'fontsize', fntsize);
ylabel('Tail probability', 'fontsize', fntsize);
...
while (1)
    ...
    %hold on;
    hnd2 = line([x1, x2], [y1, y2], 'Color', clrdat);  %changed: remove marker
...
cnt = 2;
while (1)
    hnd2 = line([x1, x2], [y1, y2], 'Color', clrfit);
    set(hnd2, 'linewidth', lnfit);
    x1 = x2; y1 = y2;
    x2 = b(bi); y2 = cf(ci);
    cnt = cnt+1;
    if(mod(cnt,2)==0)
...
hnd2 = line([x1, x2], [y1, y2], 'Color', clrfit);
set(hnd2, 'linewidth', lnfit);
legend([hnd1, hnd2],'Data','Power law','Location','SW')
