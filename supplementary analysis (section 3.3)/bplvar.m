function [alpha, bmin] = bplvar(h, boundaries, varargin)
%function [alpha, bmin, n, eof] = bplvar(h, boundaries, varargin)   %changed: commented out
...
for i=1:reps
    ...
    for xm=1:length(bmins)
        ...
        if ~isempty(rngal) 
            ...
            options = optimset('TolFun',1e-8,'TolX',1e-8,'Display','off','GradObj','on');  %changed: added the entire line
            al = fminsearch(hnd, 1, options);  %changed: added options
        end
    ...
    D = min(dat);                            %changed: commented out
    bmin = bmins(find(dat<=D, 1, 'first'));  %changed: commented out
    ...
    if ~isempty(rngal)
        ...
        hnd = @(alpha) -sum( z.*( log((l).^(1-alpha) - (u).^(1-alpha)) + (alpha-1)*log(bmin) ) );
        options = optimset('TolFun',1e-8,'TolX',1e-8,'Display','off','GradObj','on');  %changed: added the entire line
        alpha = fminsearch(hnd, 1, options);  %changed: added options
    end
...
%n = std(eof(:,1));    %changed: commented out
bmin = std(eof(:,2));
alpha = std(eof(:,3));
